...
- 比赛 Demo：富途 OpenAPI + 官方 futuapi Skill 为主力（港美股实时/推送/模拟盘）；美股期权数据方案已拍板使用富途（2026-08-12）；快照回放兜底。
...
| Alpaca 免费档 | 美股期权实时指示价、200 报价 WebSocket、纸面交易 | 仅美股；Basic 档历史受限；2026-08-12 已决定不作为正式方案（美股走富途） |
...
- 决策（2026-08-12）：美股期权数据方案使用富途；具体订阅与权限需在比赛机器实测。
