// GOAI-DSH 重跑管线插件（插件族 · Base Mode 默认）
// ---------------------------------------------------------------------------
// 工具：goai_run —— 重跑五阶段决策管线（POST /api/run，默认写审计并落盘决策卡；
// noAudit=true 时只计算不写）。verdict 可能为 NO_TRADE / BLOCK / DRAFT_ONLY /
// READY_FOR_CONFIRMATION。
// 依赖：引擎进程（127.0.0.1:8000）。本插件自带懒启动，可独立注册；推荐与
// goai-core 一起加载（Base Mode），由 core 统一托管引擎生命周期。
...
return {
  ...
  apply(ctx) {
    ...
    if (!envRoot) console.log('[goai-run] GOAI_PROJECT_ROOT 未设置，使用默认路径 ' + root + '；换机部署请先设置该环境变量再注册插件')
    ...
    function ensureEngine() {
      if (engineHandle && engineExited && !engineExternal) {
        console.log('[goai-run] 引擎进程已退出，重新拉起')
        engineHandle = null
        engineExited = false
        booting = null
      }
      ...
      booting = (async () => {
        try {
          ...
          console.log('[goai-run] 启动引擎: ' + py + ' -m src.ui_server --port ' + PORT)
