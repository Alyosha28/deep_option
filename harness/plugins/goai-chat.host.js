// GOAI-DSH 对话链路插件（插件族 · Base Mode 默认）
// ---------------------------------------------------------------------------
// 工具：goai_chat —— 自然语言 → 确定性场景解析 → 五阶段管线（全部数字与 verdict
// 来自引擎）→ 十角色辩论（LLM 只产出文字结论与证据引用，无 DeepSeek key 自动
// 离线回退）。默认写审计链。
...
return {
  ...
  apply(ctx) {
    ...
    if (!envRoot) console.log('[goai-chat] GOAI_PROJECT_ROOT 未设置，使用默认路径 ' + root + '；换机部署请先设置该环境变量再注册插件')
    ...
    function ensureEngine() {
      if (engineHandle && engineExited && !engineExternal) {
        console.log('[goai-chat] 引擎进程已退出，重新拉起')
        engineHandle = null
        engineExited = false
        booting = null
      }
      ...
      booting = (async () => {
        try {
          ...
          console.log('[goai-chat] 启动引擎: ' + py + ' -m src.ui_server --port ' + PORT)
